...
export async function seedDatabase() {
  ...
  if (existingPlansCount === 0) {
    ...
    for (const p of plansData) {
      insertPlan.run(p.amount, p.bonus);
    }
    console.log('Default buy plans seeded.');
  }

  // 4. Payment Accounts for Round-Robin
  const existingAccountsCount = db.prepare(`SELECT COUNT(*) as count FROM payment_accounts`).get().count;
  if (existingAccountsCount === 0) {
    ...
    for (const acc of accountsData) {
      insertAccount.run(acc.holder, acc.bank, acc.accNo, acc.ifsc, acc.upi, acc.limit, acc.order);
    }
    console.log('Default payment accounts seeded for Round-Robin routing.');
  }

  // 5. SEED 3 TEST USERS & SAMPLE TRANSACTIONS
  const userCheck = db.prepare(`SELECT COUNT(*) as count FROM users WHERE mobile IN ('9876543210', '9876543211', '9876543212')`).get().count;
  if (userCheck === 0) {
    ...
    db.prepare(`INSERT INTO wallet_ledger (user_id, amount, type, reference_id, description) VALUES (?, 2000, 'BUY_CREDIT', ?, 'Deposit Plan Purchase ₹2,000')`).run(user1.id, tx1Id);
    db.prepare(`INSERT INTO wallet_ledger (user_id, amount, type, reference_id, description) VALUES (?, 60, 'BUY_BONUS', ?, 'Buy Bonus (3%)')`).run(user1.id, tx1Id);
    ...
    // User 2 UPI
    db.prepare(`INSERT INTO user_upi_accounts (user_id, upi_id, status) VALUES (?, 'rahul@okicici', 'ACTIVE')`).run(user2.id);
    ...
    db.prepare(`INSERT INTO wallet_ledger (user_id, amount, type, reference_id, description) VALUES (?, 5.0, 'REFERRAL_REWARD', ?, 'Referral Reward (0.05%) from rahul456')`).run(user1.id, tx1Id);
    ...
    const user3 = db.prepare(`SELECT * FROM users WHERE mobile = '9876543212'`).get();
    db.prepare(`INSERT INTO user_upi_accounts (user_id, upi_id, status) VALUES (?, 'vikram@ybl', 'ACTIVE')`).run(user3.id);

    console.log('Seeded 3 test users successfully:');
    console.log('   - asish123 (Mobile: 9876543210 / Pass: password123) -> Balance ₹2,065.00 (Ready for Withdrawal testing)');
    console.log('   - rahul456 (Mobile: 9876543211 / Pass: password123) -> UTR Submitted (Ready for Admin UTR Verification testing)');
    console.log('   - vikram789 (Mobile: 9876543212 / Pass: password123) -> Clean user (Ready for fresh Buy/Sell testing)');
  }
}
